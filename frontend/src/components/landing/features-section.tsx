	...
	const features = [
		{
			...
			className: "md:col-span-1 lg:col-span-2 bg-gradient-to-br from-indigo-50 to-white hover:from-indigo-100 transition-colors duration-300",
		},
		{
			title: "Analytics Dashboard",
			description: "Get detailed insights into your audience growth and engagement metrics.",
			icon: <BarChart className="h-8 w-8 text-purple-500" />,
			className: "md:col-span-1 bg-gradient-to-br from-purple-50 to-white hover:from-purple-100 transition-colors duration-300",
		},
		{
			title: "Team Collaboration",
			description: "Work seamlessly with your team. Assign roles, review drafts, and approve content together.",
			icon: <Users className="h-8 w-8 text-pink-500" />,
			className: "md:col-span-1 bg-gradient-to-br from-pink-50 to-white hover:from-pink-100 transition-colors duration-300",
		},
		{
			...
			className: "md:col-span-1 lg:col-span-2 bg-gradient-to-br from-blue-50 to-white hover:from-blue-100 transition-colors duration-300",
		},
	];
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[minmax(180px,auto)]">
					{features.map((feature, idx) => (
						<motion.div
							...
							className={cn(
								"group relative overflow-hidden rounded-3xl border bg-white shadow-sm hover:shadow-md transition-all duration-300",
								feature.className
							)}
						>
							<div className="p-6 flex flex-col h-full justify-between">
								<div className="mb-4 p-3 bg-white/50 w-fit rounded-2xl shadow-sm backdrop-blur-sm border border-white/20">
									{feature.icon}
								</div>
								<div>
									<h3 className="text-xl font-semibold mb-2 text-gray-900 group-hover:text-indigo-700 transition-colors">
										{feature.title}
									</h3>
